...
export function CharacterCallScreen({ character }: { character: Character }) {
  ...
  const toggleMicrophone = (enabled: boolean): void => {
    ...
    setNotice(enabled ? 'Reconocimiento de voz no disponible en esta plataforma. Usa el chat para responder.' : 'Micrófono desactivado.');
  };
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top','right','bottom','left']}><View style={styles.page}>
    ...
    <Text style={styles.online}>● EN LÍNEA</Text><View style={styles.avatarCircle}><Text style={styles.avatar}>{character.avatar}</Text></View><Text style={styles.name}>{character.name}</Text><Text style={styles.role}>{character.personality}</Text>
