...
export function AchievementsScreen() {
  ...
  const getMetricValue = (achievementId: string, category: AchievementCategory): number => {
    switch (category) {
      case 'learning':
        return progress.leccionesCompletadas?.length ?? 0;
      case 'streak':
        return progress.rachaActual ?? 0;
      case 'chat':
        return progress.mensajesPersonajes ?? 0;
      case 'speaking':
        return progress.spokenPhrasesCount ?? 0;
      case 'travel':
        return progress.unlockedCities?.length ?? 0;
      default:
        return 0;
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.list}>
          {filteredAchievements.map((item) => {
            ...
            const currentVal = getMetricValue(item.id, item.categoria);
